  ...
  <div>
    {categories?.map((category) => (
      <div key={category.id} className="mb-8">
        ...
        {/* Scrollable row if topics > 3 */}
        <div className={`flex gap-6 ${category.topics.length > 3 ? "overflow-x-auto whitespace-nowrap flex-nowrap" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3"}`}>
          {category.topics?.map((topic) => (
            <div
              key={topic.id}
              className="cursor-pointer overflow-hidden rounded-lg shadow-lg border border-zinc-100 transform transition duration-300 hover:scale-105 min-w-[250px]"
